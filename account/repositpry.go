...
type Repository interface {
	...
	GetAccountByID(tx context.Context, id string) (*Account, error)
	ListAccounts(tx context.Context, skip uint64, take uint64) ([]Account, error)
}
...
func(r *postgresRepository) ListAccounts(ctx context.Context, skip uint64, take uint64) ([]Account, error) {
	// QueryContext — метод, который выполняет SQL-запрос и возвращает набор строк (*sql.Rows).
	...
	// при выходе из функции, освобождает ресурсы связанные с набором сторок, чтобы не было утечки памяти
	defer rows.Close()
